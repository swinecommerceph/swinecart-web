...
<div class="row">
  <div class="col s12 m4 left">
    ...
    {{-- Select All Button --}}
    <a href="#!" class="btn-floating btn-large waves-effect waves-light green tooltipped select-all-button"
      data-position="top" data-delay="50" data-tooltip="Select All Products">
      <i class="material-icons">event_available</i>
    </a>
    {{-- Display selected Button. Only show when products are hidden --}}
    @if(!empty($filters['hidden']))
    <a href="#!"
      class="btn-floating btn-large waves-effect waves-light teal lighten-2 tooltipped display-selected-button"
      data-position="top" data-delay="50" data-tooltip="Display all chosen">
      <i class="material-icons">visibility</i>
    </a>
    {{-- Hide selected Button. Only show when products are displayed --}}
    @elseif(!empty($filters['displayed']))
    <a href="#!" class="btn-floating btn-large waves-effect waves-light teal lighten-2 tooltipped hide-selected-button"
      data-position="top" data-delay="50" data-tooltip="Hide all chosen">
      <i class="material-icons">visibility_off</i>
    </a>
    @endif
    {{-- Delete selected Button --}}
    <a href="#!" class="btn-floating btn-large waves-effect waves-light red darken-4 tooltipped delete-selected-button"
      data-position="top" data-delay="50" data-tooltip="Delete all chosen">
    ...
    <div class="row">
      <div id="sort-select" class="input-field col s12 m4 l3 right">
        <select>
          ...
          <option value="birthdate-desc" @if(!empty($filters['birthdate-desc'])) {{ $filters['birthdate-desc'] }}
            @endif>Age: Low to High</option>
          <option value="adg-desc" @if(!empty($filters['adg-desc'])) {{ $filters['adg-desc'] }} @endif>Average Daily
            Gain</option>
          <option value="fcr-desc" @if(!empty($filters['fcr-desc'])) {{ $filters['fcr-desc'] }} @endif>Feed Conversion
            Ratio</option>
      ...
      <div id="status-select" class="input-field col s12 m4 l3 right">
        <select>
          ...
          <option value="displayed" @if(!empty($filters['displayed'])) {{ $filters['displayed'] }} @endif>Displayed
          </option>
          <option value="hidden" @if(!empty($filters['hidden'])) {{ $filters['hidden'] }} @endif>Hidden</option>
          <option value="requested" @if(!empty($filters['requested'])) {{ $filters['requested'] }} @endif>Requested
          </option>
        </select>
        <label>Status</label>
      </div>
...
<div id="view-products-container" class="row grey lighten-4">
  ...
  <div class="col s12 m6 l4" id="product-{{$product->id}}">
    <div class="card hoverable" style="border: solid 4px transparent;">
      ...
      <div class="card-action" style="background: hsl(0, 0%, 95%); border-top: none !important;">
        <div class="row">
          <div class="col left">
            <input type="checkbox" id="check-{{$productNumber}}" data-product-id="{{$product->id}}"
              class="filled-in single-checkbox" />
            <label for="check-{{$productNumber}}"></label>
          </div>
          <div class="col right">
            {{-- Edit Button --}}
            <a href="{{ route('products.editProduct', ['product' => $product->id]) }}"
              class="tooltipped edit-product-button" data-position="top" data-delay="50"
              data-tooltip="Edit '{{$product->name}}'" data-product-id="{{$product->id}}">
              <i class="material-icons primary-text" style="font-size:30px">edit</i>
            </a>
            @if(!empty($filters['hidden']) || $product->status == 'hidden')
            {{-- Display Button --}}
            <a href="#!" class="tooltipped display-product-button" data-position="top" data-delay="50"
              data-tooltip="Display '{{$product->name}}'" data-product-id="{{$product->id}}"
            ...
            {{-- Hide Button --}}
            <a href="#!" class="tooltipped hide-product-button" data-position="top" data-delay="50"
              data-tooltip="Hide '{{$product->name}}'" data-product-id="{{$product->id}}"
              data-product-name="{{$product->name}}">
              <i class="material-icons primary-text" style="font-size:30px">visibility_off</i>
            </a>
            @endif
            {{-- Delete Button --}}
            <a href="#!" class="tooltipped delete-product-button" data-position="top" data-delay="50"
              data-tooltip="Delete '{{$product->name}}'" data-product-id="{{$product->id}}">
      ...
      <div class="card-reveal">
        ...
        <div class="row">
          ...
          <br> --}}
          <div class="col">
            <a href="{{ route('products.bViewDetail', ['product' => $product->id]) }}" class="waves-effect waves-light"
              style="
