...
@section('content')
    <div class="row valign-wrapper">
        <div class="col s12 m6 l6 valign">
            <select onChange="window.location.href=this.value">
                <option selected value="{{route('admin.statistics.transactions')}}">Monthly Transactions</option>
                <option value="{{route('admin.statistics.totaltransactions')}}">Total Completed Transactions</option>
            </select>
        </div>
        <div class="col s12 m6 l6">
            {!!Form::open(['route'=>'admin.statistics.transactions-date', 'method'=>'GET', 'class'=>'row valign-wrapper'])!!}
                <div class="col s8 m8 l8 valign">
                    ...
                    <input type="number" name="year" min="2000" max="{{ $now[2] }}" value="{{ $year }}">
                </div>
